...
void static SciiaMiner(const CChainParams& chainparams)
{
    LogPrintf("SciiaMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sciia-miner");
    ...
    CWallet * pWallet = NULL;
        ...
        if (!EnsureWalletIsAvailable(pWallet, false)) {
            LogPrintf("SciiaMiner -- Wallet not available\n");
        }
    #endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("SciiaMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("SciiaMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("SciiaMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("SciiaMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateSciias(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&SciiaMiner, boost::cref(chainparams)));
    }
    return(numCores);
}
