...
const char * const BITCOIN_CONF_FILENAME = "sciia.conf";
const char * const BITCOIN_PID_FILENAME = "sciiad.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "sciiad -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SciiaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SciiaCore
    // Mac: ~/Library/Application Support/SciiaCore
    // Unix: ~/.sciiacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SciiaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SciiaCore";
#else
    // Unix
    return pathRet / ".sciiacore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty sciia.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Sciia Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Sciia Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Sciia Core developers";
    }
