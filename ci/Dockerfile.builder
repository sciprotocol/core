...
# sciia_hash
RUN git clone https://github.com/raptor3um/sciia_hash
RUN cd sciia_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} sciia
RUN useradd -u ${USER_ID} -g sciia -s /bin/bash -m -d /sciia sciia
...
RUN mkdir /sciia-src && \
  ...
  chown $USER_ID:$GROUP_ID /sciia-src && \
