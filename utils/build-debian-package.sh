...
PACKAGE_VERSION=$($SRC_PATH/src/sciiad --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/sciiad $DEB_BIN
cp $SRC_PATH/src/sciia-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/sciia-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/sciiad.1 $DEB_MAN
cp $SRC_DOC/man/sciia-cli.1 $DEB_MAN
cp $SRC_DOC/man/sciia-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/sciiad.bash-completion $DEB_CMP/sciiad
cp $SRC_PATH/contrib/sciia-cli.bash-completion $DEB_CMP/sciia-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/sciiad.1
gzip --best -n $DEB_MAN/sciia-cli.1
gzip --best -n $DEB_MAN/sciia-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/sciiad $DEB_BIN/sciia-cli
