...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/SciiaParams"
else
    PARAMS_DIR="$HOME/.sciia-params"
fi
...
DOWNLOAD_URL="https://github.com/sciprotocol/core/releases/tag/1.3.17.03"
...
# fetch methods can be disabled with SCI_DISABLE_SOMETHING=1
SCI_DISABLE_WGET="${SCI_DISABLE_WGET:-}"
#SCI_DISABLE_IPFS="${SCI_DISABLE_IPFS:-}"
SCI_DISABLE_CURL="${SCI_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$SCI_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$SCI_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$SCI_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Sciia parameters!
...
This script will fetch the Sciia SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Sciia SNARK parameters. Note that it is
